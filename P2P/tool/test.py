...
def test_cls(model, val_data_loader):
    ...
    with torch.no_grad():
        ...
        for i, batch_data in enumerate(val_data_loader):
            ...
            dataset_mult.append(batch_data[2].numpy())
        ...
        dataset_mult = np.concatenate(dataset_mult)
        ...
        df = pd.DataFrame({'pred': preds, 'gts': labels, 'dataset': dataset_mult})
        df.to_csv('preds.csv')

        oAccs = []
        wAcc = 0
        for m in range(1, 5):
            mlabels = np.extract(dataset_mult == m, labels)
            mpreds = np.extract(dataset_mult == m, preds)
            moAcc = metrics.accuracy_score(mlabels, mpreds) * 100
        ...
        if main_process():
            print(oAccs)
            print("Overall weighted accuracy (competition scoring):", wAcc) 
